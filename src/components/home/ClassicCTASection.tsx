      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-10">
            {benefits.map((b) => (
              <motion.div key={b} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="flex items-center gap-2 px-4 py-2 rounded-full bg-primary-foreground/10 backdrop-blur-sm border border-primary-foreground/20">
                <CheckCircle className="w-4 h-4 text-travel" />
                <span className="text-sm font-medium text-primary-foreground/90">{b}</span>
              </motion.div>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button variant="contribution" size="xl" onClick={() => navigate("/login/contribution")} className="group">
              Join Amana Contribution
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Button>
            <Button variant="travel" size="xl" onClick={() => navigate("/teemah-travels")} className="group">
